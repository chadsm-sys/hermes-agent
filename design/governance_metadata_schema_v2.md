...
3. `authority_decision=approval_required|denied` must not coexist with `recommendation_category=allow_simulation_only` unless the recommendation is explicitly classified as an error during adjudication.
...
5. `contract_resolution_status` other than `resolved|not_applicable` must not produce an allow category.
6. `evidence_sufficiency=insufficient|indeterminate|evidence_invalid` must not produce an allow category.
...
9. All unknown/indeterminate values remain admissible for observation but make the record ineligible for a definitive correctness label unless the recommendation is an appropriate abstention/denial.
