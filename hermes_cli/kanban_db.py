    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # Olympus guards persist in sqlite_master, but their permit
                # predicates are connection-local UDFs and must be installed
                # on every steady-state connection as well as first open.
...
def release_stale_claims(
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    for row in stale:
        ...
        # Preserve the live runtime's duplicate-worker guard on the legacy
        # path. Governed recovery above uses its durable staged effect model.
